...
Заполненный [опросник и файлы](https://github.com/Roma-EDU/devops-netology/tree/master/mnt-homeworks/08-ansible-01-base) из 12 пункта - в отдельной папке (вместо репозитория, пока так удобнее). Но тут на Шаге 2 Подготовки возник вопрос, просьба подсказать. И с bash-скриптом из необязательной части тоже
...
1. Запустите playbook на окружении `prod.yml`. При запуске `ansible` должен запросить у вас пароль. Убедитесь что факты `some_fact` для каждого из хостов определены из верных `group_vars`.
   * Снова запустил `ansible-playbook -i inventory/prod.yml site.yml --ask-vault-pass`, убедился что появился новый хост `localhost` со значением переменной some_fact из группы all `all default fact`
   ```bash
   $ ansible-playbook -i inventory/prod.yml site.yml --ask-vault-pass
   Vault password:
...
1. Заполните `README.md` ответами на вопросы. Сделайте `git push` в ветку `master`. В ответе отправьте ссылку на ваш открытый репозиторий с изменённым `playbook` и заполненным `README.md`.
   * Дублирую ссылку из начала [опросник и файлы](https://github.com/Roma-EDU/devops-netology/tree/master/mnt-homeworks/08-ansible-01-base)

## Необязательная часть

1. При помощи `ansible-vault` расшифруйте все зашифрованные файлы с переменными.
   * Для этого вызовем команду `ansible-vault decrypt ./group_vars/el/examp.yml` и введём пароль `netology`, который использовали при шифровании
1. Зашифруйте отдельное значение `PaSSw0rd` для переменной `some_fact` паролем `netology`. Добавьте полученное значение в `group_vars/all/exmp.yml`.
   * Выполнено ранее в пункте 7, cм. `ansible-vault encrypt_string` 
